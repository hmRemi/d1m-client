package com.d1m.client.hud.altmanager;
...
import com.d1m.client.hud.altmanager.components.GuiAccountList;
import com.d1m.client.hud.altmanager.impl.GuiAddAlt;
import com.d1m.client.hud.altmanager.impl.GuiAltLogin;
import com.d1m.client.hud.altmanager.thealtening.AltService;
import com.d1m.client.hud.altmanager.thread.AccountLoginThread;
...
public class GuiAltManager extends GuiScreen {
	...
	public void drawScreen(int p_drawScreen_1_, int p_drawScreen_2_, float p_drawScreen_3_) {
		...
		if (this.loginThread != null)
			...
			drawCenteredString(this.mc.fontRendererObj, "Accounts: " + D1m.instance.getAccountManager().getAccounts().size(), this.width / 2, 20, -1);
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		case 4:
			...
			break;
		case 7:
			if (D1m.instance.getAccountManager().getAccounts().size() == 0)
				return;

			registry = D1m.instance.getAccountManager().getAccounts();
			random = new Random();
			randomAlt = registry.get(random.nextInt(D1m.instance.getAccountManager().getAccounts().size()));
		...
		case 5:
			...
			if (returnVal == 0) {
				...
				try {
					for (String line : Files.readAllLines(Paths.get(chooser.getSelectedFile().getPath(), new String[0]))) {
						...
						D1m.instance.getAccountManager().getAccounts().add(account);
					}
			...
			break;
		case 8:
			if (D1m.instance.getAccountManager().getLastAlt() == null)
				return;
			this.loginThread = new AccountLoginThread(D1m.instance.getAccountManager().getLastAlt().getEmail(), D1m.instance.getAccountManager().getLastAlt().getPassword());
			this.loginThread.start();
			break;
		case 9:
			if (D1m.instance.getAccountManager().getAccounts().isEmpty())
				return;
			D1m.instance.getAccountManager().getAccounts().clear();
			break;
		}
	}
